...
<body>
  ...
  <script>
      ...
      return \`<div class="card" id="card-\${u.id}">
        ...
        <div class="photos">
          <div>
            ...
            <div class="photo-box">\${idUrl ? \`<img src="\${idUrl}" onclick="window.open(this.src)" onerror="this.parentElement.innerHTML='<div class=no-photo>Photo deleted or not found</div>'" />\` : '<div class="no-photo">No photo</div>'}</div>
          </div>
          <div>
            <div class="photo-label">USPS Mail</div>
            <div class="photo-box">\${mailUrl ? \`<img src="\${mailUrl}" onclick="window.open(this.src)" onerror="this.parentElement.innerHTML='<div class=no-photo>Photo deleted or not found</div>'" />\` : '<div class="no-photo">No photo</div>'}</div>
          </div>
        </div>
        <p class="delete-notice">⚠️ Photos are permanently deleted from server after you approve or reject.</p>
        \${status === 'pending' ? \`
        <div class="actions">
    ...
    function renderVehicle(v) {
      ...
      const hasPrefilledLocation = !!(v.meeting_location_name);
      if (hasPrefilledLocation) selectedLocations[v.id] = { name: v.meeting_location_name, address: v.meeting_location_address || '' };

      return \`<div class="card" id="vcard-\${v.id}">
        <div class="card-header"><span class="card-id">Listing #\${v.id} — \${v.seller_dvvia_id || 'Unknown'}</span><span class="badge \${badgeClass}">\${status}</span></div>
        ...
        <div class="form-grid">
          ...
          <div class="form-group"><label>Mileage</label><input type="number" id="v\${v.id}-mileage" placeholder="e.g. 34200" value="\${v.mileage > 0 ? v.mileage : ''}" /></div>
          ...
          <div class="form-group"><label>Title Status</label><select id="v\${v.id}-title"><option value="Clean" \${v.title_status==='Clean'?'selected':''}>Clean</option><option value="Salvage" \${v.title_status==='Salvage'?'selected':''}>Salvage</option><option value="Rebuilt" \${v.title_status==='Rebuilt'?'selected':''}>Rebuilt</option><option value="Lien" \${v.title_status==='Lien'?'selected':''}>Lien</option></select></div>
          ...
          <div class="form-group"><label>Transmission</label><select id="v\${v.id}-trans"><option value="Automatic" \${v.transmission==='Automatic'?'selected':''}>Automatic</option><option value="Manual" \${v.transmission==='Manual'?'selected':''}>Manual</option><option value="CVT" \${v.transmission==='CVT'?'selected':''}>CVT</option></select></div>
        </div>

        \${isPending ? \`
        <div class="location-section">
          <div class="location-title">📍 Meeting Location</div>
          \${hasPrefilledLocation ? \`<div class="location-prefilled">✓ Seller chose: \${v.meeting_location_name}\${v.meeting_location_address ? '<br><span style="font-size:12px;font-weight:400;color:rgba(0,200,150,0.7)">' + v.meeting_location_address + '</span>' : ''}</div><p style="font-size:12px;color:#5a6a80;margin-bottom:12px;">You can override with a different spot if needed:</p>\` : \`<p style="font-size:12px;color:#5a6a80;margin-bottom:12px;">Seller did not choose a location. Enter their verified zip code to set one:</p>\`}
        ...
        <div class="actions">
          <button class="btn btn-approve" id="v\${v.id}-approve-btn" onclick="approveVehicle(\${v.id})" \${hasPrefilledLocation ? '' : 'disabled'}>✓ Approve & Go Live</button>
          <button class="btn btn-red" onclick="rejectVehicle(\${v.id})">✕ Reject Listing</button>
        </div>\` : \`<div style="color:#00c896;font-weight:700;margin-top:8px;">\${status === 'active' ? '✓ Live — ' + (v.meeting_location_name || '') : '✕ Rejected'}</div>\`}
      </div>\`;
    }
    ...
    async function findNearbySpots(vehicleId) {
      ...
      try {
        ...
        if (!geoData || geoData.length === 0) { spotsDiv.innerHTML = '<div class="location-error">⚠️ Zip code not found.</div>'; findBtn.disabled = false; return; }
        ...
        const overpassRes = await fetch('https://overpass-api.de/api/interpreter', { method: 'POST', body: 'data=' + encodeURIComponent(q) });
    ...
    function approveUser(userId) {
      if (!confirm('Approve this identity? Their photos will be permanently deleted from the server.')) return;
    ...
    function rejectUser(userId) {
      ...
      if (!confirm('Reject this user? Their photos will be permanently deleted from the server.')) return;
    ...
    function approveVehicle(vehicleId) {
      ...
      if (!location) { alert('Please select a meeting location before approving.'); return; }
      fetch('/api/admin/vehicles/' + vehicleId + '/approve', { method: 'POST', headers: { 'x-admin-password': currentPwd, 'Content-Type': 'application/json' }, body: JSON.stringify({ year, make, model, mileage, vin, titleStatus, exteriorColor: color, transmission, meetingLocationName: location.name, meetingLocationAddress: location.address }) })
        .then(r => r.json()).then(d => {
          if (d.success) { document.getElementById('vcard-' + vehicleId).style.opacity = '0.5'; document.getElementById('vcard-' + vehicleId).querySelector('.actions').innerHTML = '<span style="color:#00c896;font-weight:700;">✓ Live — ' + location.name + '</span>'; loadCounts(); }
...
app.post('/api/auth/register', uploadReg.fields([{ name: 'id_photo', maxCount: 1 }, { name: 'mail_photo', maxCount: 1 }]), async (req, res) => {
  try {
    const pool = getDb();
    ...
    const result = await pool.query(`INSERT INTO users (dvvia_id, password_hash, id_photo_path, mail_photo_path, verification_status) VALUES ($1, $2, $3, $4, 'pending') RETURNING id`, [dvviaId, hash, idPhotoPath, mailPhotoPath]);
...
app.get('/api/admin/vehicles/pending', adminAuth, async (req, res) => {
  try {
    const pool = getDb();
    const result = await pool.query(`SELECT v.*, u.dvvia_id as seller_dvvia_id FROM vehicles v JOIN users u ON v.seller_id = u.id WHERE v.status != 'active' OR v.title_status = 'Pending Review' ORDER BY v.created_at DESC`);
...
app.post('/api/admin/vehicles/:id/approve', adminAuth, async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [year, make, model, mileage, vin, titleStatus || 'Clean', exteriorColor, transmission, meetingLocationName, meetingLocationAddress, vid]
    );
...
app.post('/api/vehicles', uploadVehicle.fields([{ name: 'title_photo', maxCount: 1 }, { name: 'vin_photo', maxCount: 1 }, { name: 'odometer_photo', maxCount: 1 }]), async (req, res) => {
  try {
    const pool = getDb();
    const b = req.body;
    ...
    const result = await pool.query(
      ...
      [b.sellerId, b.vin || 'PENDING', b.year, b.make, b.model, b.trimLevel, b.price, b.mileage, b.transmission, b.drivetrain, b.fuelType, b.engine, b.exteriorColor, b.interiorColor, b.titleStatus || 'Pending Review', b.conditionExterior, b.conditionInterior, b.conditionTires, b.conditionMechanical, b.conditionAc, b.conditionElectronics, titlePhotoPath, vinPhotoPath, odometerPhotoPath, b.meetingLocationName || null, b.meetingLocationAddress || null]
    );
    res.json({ success: true, vehicleId: result.rows[0].id });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});

// Public listings — never return sensitive photo paths
